...
function exerciseLogger(exercise: Exercise, sessionSets: SetEvent[]): string {
  ...
  return `<article class="exercise-sheet" data-exercise="${escapeHtml(exercise.id)}">
    <div class="exercise-title"><div><p class="set-kicker">${done ? 'Plan complete · add another if needed' : `Set ${next} of ${exercise.targetSets}`}</p><h2>${escapeHtml(exercise.name)}</h2></div><ol class="set-dots" aria-label="${sets.length} completed of ${exercise.targetSets} planned">${Array.from({ length: Math.max(exercise.targetSets, sets.length) }, (_, index) => `<li class="${index < sets.length ? 'done' : ''}">${index < sets.length ? '✓' : index + 1}</li>`).join('')}</ol></div>
...
function showRoutineDialog(routine?: Routine): void {
  ...
  dialog.innerHTML = `<form id="routine-form" method="dialog" data-id="${escapeHtml(routine?.id ?? '')}">
    <div class="dialog-heading"><div><p class="eyebrow">Routine</p><h2 id="routine-dialog-title">${routine ? 'Edit routine' : 'New routine'}</h2></div><button class="icon-button" type="button" data-action="close-dialog" aria-label="Close routine form">×</button></div>
